<template>
	<div>
		<a-button @click="modalVisible = true">
			<ui-icon iconType="ant-design" type="link" color="#333333" />
			Add contract
		</a-button>
		...
		<a-table
			...
			:loading="isLoading"
			:pagination="false"
			key="table-game-contract"
		>
			<!--	 title		-->
			<div class="game-contract-table__title" slot="game-contract_title">
				Game Contracts
			</div>
...
export default Vue.extend({
	...
	methods: {
		...
		...mapActions({
			fetchList: "parsing-jobs/fetchGameContractList",
			fetchGameList: "games/fetchGames",
			saveContract: "parsing-jobs/saveGameContract",
			deleteContract: "parsing-jobs/deleteGameContract"
		}),
		setWidth(): void {
